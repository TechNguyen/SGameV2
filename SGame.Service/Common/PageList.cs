...
namespace SGame.Common
{
	public class PageList<T>
	{
        ...
        public static PageList<T> Create(IQueryable<T> query,SearchBase search) {
        ...
        public static PageList<T> Create(List<T> query, SearchBase search)
        {
            var totalCount = query.Count();
            var items = query.Skip((search.PageIndex - 1) * search.PageSize).ToList();
            var totalPage = (int)Math.Ceiling((double)totalCount / search.PageSize);
            return new PageList<T>(items, search.PageIndex, search.PageSize, totalCount, totalPage);
        }
    }
